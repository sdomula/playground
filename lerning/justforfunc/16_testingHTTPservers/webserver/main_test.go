...
func TestDoubleHanlder(t *testing.T) {
	tt := []struct {
		name   string
		value  string
		double int
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			b := strings.TrimSpace(rec.Body.String())
			...
			if res.StatusCode != http.StatusOK {
				t.Errorf("expacted status %v; got %v", tc.status, res.StatusCode)
			}
...
func TestRouting(t *testing.T) {
	...
	if res.StatusCode != http.StatusOK {
		t.Errorf("expacted status %v; got %v", http.StatusOK, res.StatusCode)
	}
